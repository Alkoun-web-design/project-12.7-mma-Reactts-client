...
const CounsellorDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      setLoading(true);
      try {
        const res = await fetch(`${serverAPI}counsellors/dashboard/${id}`, { credentials: 'include' });
  ...
  const handleAvatarChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      if (avatar instanceof File) {
        ...
        try {
          const res = await fetch(`${serverAPI}counsellors/dashboard/${id}`, { 
            method: 'PUT',
            credentials: 'include', 
            body: formData
          });
          ...
          <div className="w-full bg-white rounded-lg shadow p-8">
            ...
            <form onSubmit={handleSubmit} encType="multipart/form-data" className="text-sm space-y-5">
              ...
              <div>
                ...
                <input name="password" value={form.password} onChange={handleChange} type="password" className="w-full border rounded px-3 py-2" minLength={8} maxLength={24} placeholder="Change your password" title="Minimum = 8, Maximum = 24"/>
              </div>
              <div className="pt-2">
