...
const CounsellorProfile: React.FC = () => {
  ...
  const [counsellor, setCounsellor] = useState<CounsellorMember | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchCounsellorMember = async () => {
      try {
        const response = await fetch(`${serverAPI}counsellors/${id}`, {credentials: 'include'});
        if (!response.ok) {
          throw new Error('Failed to fetch counsellor member');
        }
        const data = await response.json();
        setCounsellor(data);
      } catch (err) {
        console.error('Error fetching counsellor member:', err);
      } finally {
        setLoading(false);
      }
    };

    fetchCounsellorMember();
  }, [id]);
    ...
    return (
      <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-2xl font-bold text-red-600 mb-4">{ 'Counsellor member not found'}</h2>
          <button
            onClick={() => navigate('/our-staff')}
            className="btn btn-primary"
          >
            Return to Counsellor List
          </button>
        </div>
      </div>
    );
        ...
        >
          <div className="relative h-64 sm:h-80">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
              <h1 className="text-3xl font-bold mb-2">{counsellor?.name}</h1>
            </div>
          </div>
          ...
          <div className="p-6 sm:p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                ...
                <p className="text-gray-600 mb-8">{counsellor?.bio}</p>
                ...
                {counsellor?.education && (
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600">{counsellor?.education}</p>
                    </div>
                  </div>
                )}

                {counsellor?.work_experience && (
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600">{counsellor?.work_experience}</p>
                    </div>
                  </div>
                )}
                ...
                {counsellor?.certifications && (
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600">{counsellor?.certifications}</p>
                    </div>
                  </div>
                )}
                ...
                {counsellor?.languages && (
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600">{counsellor?.languages}</p>
                    </div>
                  </div>
                )}
                ...
                {counsellor?.achievements && (
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600">{counsellor?.achievements}</p>
                    </div>
                  </div>
                )}
